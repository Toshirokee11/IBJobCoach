...
$(document).ready(function() {
    ...
    function txtMes(num) {
        var arrnum = ["cero", "un", "dos", "tres", "cuatro", "cinco", "seis", "siete", "ocho", "nueve"];
        return arrnum[num];
    }

    function paypal(dl) {
        var arrpaypal = ["cero", "JFKTE4U6XXN5C", "MN5PY245EDQLU", "M6N8D27ANKCLS", "LGA2Q2QYB4UDC", "LEMMGSFVPNFZ8", "FJGKL42T3QD3Q", "X2YUVPDY923FL", "QKREVUYKQXGPE"];
        return arrpaypal[dl];
    }
    ...
    function monedaCL(mon) {
        var arrmonCL = ["0", "60.000", "54.000", "48.000", "43.000", "39.000", "30.000"];
        return arrmonCL[mon];
    }
    ...
    function monedaCO(mon) {
        var arrmonCL = ["0", "220.000", "195.000", "175.000", "160.000", "145.000", "110.000"];
        return arrmonCL[mon];
    }
    ...
    if (index > 0) {
        ...
        var valorDsctoDolar = Math.round(valorDolar - (valorDolar * (porcDscto / 100)));
        ...
        $('#cargos > option[value="' + valor_select + '"]').attr('selected', 'selected');
...
$(document).ready(function() {
    ...
    $('[name = "phone"]').intlTelInput({
        onlyCountries: ["ar", "br", "cl", "us", "pe", "co", "mx", "bo"],
        preferredCountries: ["pe"],
        nationalMode: true,
    });

    $(".cargos_programa option").filter(function() {
    ...
    $('[name = "phone"]').intlTelInput("setCountry", getCountryCode(window.genesCountry));
...
function setCountryMask(country) {
    ...
    switch (country["iso2"]) {
        ...
        case "pe":
            $('[name = "phone"]').inputmask({ "mask": "+51 999-999-999" });
            break;
...
$(function() {
    $("#formplanes").on("submit", function(e) {
        ...
        $.ajax({
            ...
            success: function(datos) {
                window.location = datos;
                window.location.href = ('paypal/index.php');
            },
            error: function(datos) {
                var mensaje = "El no se pudo registrar"
